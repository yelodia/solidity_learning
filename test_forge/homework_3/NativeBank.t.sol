...
contract NativeBankTest is Test, Accounts {
    ...
    function setUp() public {
        ...
        bank = new NativeBank([signers[1].addr, signers[2].addr, signers[3].addr]);
    }
    ...
    function test_Deposit_InsufficientDeposit() public {
        vm.expectRevert(
            abi.encodeWithSelector(NativeBank.InsufficientDeposit.selector, signers[5].addr)
        );
    ...
    function test_SetCommission_NotByOwner() public {
        vm.expectRevert(NativeBank.NotContractOwner.selector);
        vm.prank(signers[5].addr);
        bank.setCommission(150);
    }
